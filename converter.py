...
def feats_to_dict(univfeature, univfeature_dict):
    """
...
def parse(token, lemma, emmorph):
    ...
    # a 'meg' igekötő kezelése
        ...
        feats_to_dict(univfeature, univfeature_dict)

        # ha az infinitívusz ragozott, akkor tense jegye is van
        if 'VerbForm' in univfeature_dict:
            if univfeature_dict['VerbForm'] == 'Inf' and 'Person' in univfeature_dict and 'Number' in univfeature_dict:
                univfeature_dict['Tense'] = 'Pres'
    ...
    return univpos, print_feats(univfeature_dict)
...
def main():
    ...
    for line in sys.stdin:
        ...
        univpos, univfeature = parse(token, lemma, elemzes)
